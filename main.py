...
port_number = 10000  # Start using ports from port number 8000
host = 'localhost'
...
class LoadBalancer:
    ...
    def LeastBandwidth(self):
        q = queue.PriorityQueue()
        for i in range(self.n):
            q.put((-1 * (self.bandwidth[i] + self.capacities[i]), i))
        i = 0
        while i < self.m:
            if len(self.timers) > 0 and time.time() >= self.timers[0][0] + self.connection_timeout:
                ...
                temp = self.capacities[b] - len(self.servers[f"Server{b + 1}"])
                print(f"Bandwidth of server{b} changed to {-1 * self.bandwidth[b] / max(temp, 1)}")
                q.put((-1 * (self.bandwidth[b] + self.capacities[b]) / max(temp, 1), b))
            ...
            if len(self.servers[f"Server{server_number + 1}"]) > 0:
                ...
                self.timers.append((time.time(), server_number, port, f"client{i}"))
                Client(port)
                ...
                self.events[f"client{i}"] = (server_number, port)
                q.put(
                    (-1 * (self.bandwidth[server_number] + self.capacities[server_number]) / (temp + 1), server_number))
                self.nlb_logs.append(
                    f"Redirected client {i} request to server {server_number}")
                i += 1
            else:
                self.nlb_logs.append(f"Redirection of client {i} request to server {server_number} failed (No capacity left)")
    ...
    def StaticLeastConnections(self):
        q = queue.PriorityQueue()
        Sum = 0
        for i in range(self.n):
        ...
        while Sum < self.m:
            ...
            if len(self.servers[f"Server{server_number + 1}"]) > 0:
                ...
                self.nlb_logs.append(
                    f"Redirected client {Sum} request to server {server_number}")
                Sum += 1
                q.put((-1 * (self.capacities[server_number] - temp), server_number))
            else:
                self.nlb_logs.append(
                    f"Redirection of client {Sum} request to server {server_number} failed (No capacity left)")
...
class Simulate:
    ...
    def DisplayMenu(self):
        ...
        btn = tk.Button(self.root, text="Dynamic Round Robin", font=('Century Gothic', 18), command=self.DRR)
        btn.place(x=30, y=190)
        btn.config(width=28)
        btn = tk.Button(self.root, text="Least Connection", font=('Century Gothic', 18),
                        command=self.SLC)
        btn.place(x=30, y=280)
        btn.config(width=28)
        ...
        btn.place(x=30, y=460)
        btn.config(width=28)
        btn = tk.Button(self.root, text=" Least Bandwidth Load Balancing", font=('Century Gothic', 18), command=self.LB)
        btn.place(x=30, y=550)
        btn.config(width=28)
        btn = tk.Button(self.root, text="Content-Aware Load Balancing", font=('Century Gothic', 18), command=self.CA)
        btn.place(x=30, y=640)
        btn.config(width=28)
        self.root.mainloop()
    ...
    def Run(self, option):
        ...
        elif option == "leastBandwidth":
            self.get_capacities(canvas, True)
            self.lb.LeastBandwidth()
            self.RunDynamicRoundRobin(canvas)
        elif option == "iphash":
